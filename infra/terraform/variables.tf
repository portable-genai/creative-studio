...
variable "guardrail_enabled" {
  description = "Switch the input and output guardrail (Model Armor under gcp) on the service (MKT_CREATIVE_GUARDRAIL). A cheap runtime control: on in the reference, reversible, so it takes a default."
  type        = bool
  default     = true
}

variable "review_routing_enabled" {
  description = "Switch the hand-off of every creative result to the human-review-console (MKT_CREATIVE_REVIEW_ROUTING). A cheap runtime control: on in the reference, reversible, so it takes a default."
  type        = bool
  default     = true
}
...
variable "human_review_url" {
  description = <<-EOT
    The human-review-console base URL the review router submits every creative result to
    ...
    then may pass ""). HTTPS, because the payload carries the creative result.
  EOT
  type        = string
