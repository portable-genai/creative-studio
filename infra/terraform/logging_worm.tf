...
#     is var.retention_days (~7 years) and whose `locked = true` makes it Write-Once-Read-
...
resource "google_logging_project_bucket_config" "worm_audit" {
  ...
  description    = "WORM audit bucket for D3 creative studio (locked, ~7y retention)."
  ...
  cmek_settings {
    kms_key_name = google_kms_crypto_key.creative.id
  }
...
resource "google_logging_project_sink" "audit_to_worm" {
  ...
  description = "Routes the creative-studio-audit log to the locked WORM bucket."
