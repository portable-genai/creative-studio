...
#
# Signals covered:
#   - guardrail_blocks : a guardrail BLOCKED decision in the app audit log (Model Armor /
#                        brand-safety guardrail working). Matches the audit payload the gcp
#                        audit adapter writes for a blocked creative.
...
locals {
  security_metrics = {
    guardrail_blocks = {
      ...
      filter      = "logName=\"projects/${var.project_id}/logs/creative-studio-audit\" AND jsonPayload.decision=\"blocked\""
    }
    sa_key_creation = {
...
resource "google_monitoring_alert_policy" "security" {
  ...
  project      = var.project_id
  display_name = "D3 security: ${each.key}"
  combiner     = "OR"
  ...
  documentation {
    content   = "Security signal '${each.key}' fired for the D3 creative studio. Investigate the matching entries in Cloud Logging and the WORM audit bucket."
