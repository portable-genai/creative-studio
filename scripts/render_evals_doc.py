...
GOLDEN = _REPO_ROOT / "eval" / "datasets" / "golden_creative.jsonl"
...
def _metrics_block() -> list[str]:
    ...
    lines += ["", f"Scored over {len(cases)} golden creative briefs.", ""]
    return lines


def _exercised_block() -> list[str]:
    briefs = load_jsonl(GOLDEN)
    narrative = load_jsonl(NARRATIVE)
    return [
        BLOCKS[1],
        "",
        f"- **{len(briefs)} golden creative briefs** in `eval/datasets/golden_creative.jsonl`,",
        ...
        f"- **{len(narrative)} judged copy cases** in `eval/datasets/narrative_golden.jsonl`, each",
