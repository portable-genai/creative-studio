...
Rewrites the package name, the CLI entry point, the ``MKT_CREATIVE_`` env-var prefix, and the
...
changed), then run the gate. See docs/ADOPTING.md for the full checklist (seed corpus,
fixtures, brand / claim / policy rules, IdP wiring, region) that this script does NOT do for
you.
"""
...
_OLD_PACKAGE = "creative_studio"
_OLD_CLI = "mkt-creative"
_OLD_ENV_PREFIX = "MKT_CREATIVE_"
_OLD_RESOURCE = "creative-studio"
_OLD_DIST = "creative-studio"
...
def _replacements(args: argparse.Namespace) -> list[tuple[str, str]]:
    ...
    # Order matters: the distribution name and the resource stem are the same string here,
    # and the CLI name is a substring of the resource stem, so replace the longer, more
    # specific strings first. The distribution defaults to the resource stem (they match).
    return [
...
def _rewrite_text(text: str, repls: list[tuple[str, str]]) -> tuple[str, int]:
    ...
    for old, new in repls:
        # For the env prefix, only rewrite ``MKT_CREATIVE_`` when it prefixes an UPPER
        # token (MKT_CREATIVE_PROFILE), never mid-word.
...
def main() -> int:
    ...
    ap.add_argument(
        "--resource", required=True, help="new resource id stem, e.g. acme-creative-studio"
    )
    ...
    print("Then finish the human steps in docs/ADOPTING.md (seed corpus, rules, IdP, region).")
    return 0
