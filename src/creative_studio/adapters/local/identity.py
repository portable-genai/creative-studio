...
settings came from the environment) ``MKT_CREATIVE_PROFILE`` or the settings file must have
named it, rather than it being inherited from a fallback. A missing variable therefore fails
closed instead of serving a brand-copy API to any caller as a seeded creative approver.

The entitlement group names are D3's creative-studio domain (creative analyst / approver,
brand and audit teams); the persona ids, subjects and tenants (including the cross-tenant
one) are shared across the catalog so per-tenant authorization is demoable offline.
"""
...
_PERSONAS: tuple[Principal, ...] = (
    Principal(
        ...
        principals=("group:creative-analyst", "group:brand"),
        tenant="demo-bank",
    ...
    Principal(
        ...
        principals=("group:creative-analyst", "group:brand", "group:creative-approver"),
        tenant="demo-bank",
        assurance="local-demo",
    ...
    Principal(
        ...
        principals=("group:creative-analyst",),
        tenant="other-bank",
        assurance="local-demo",
...
class LocalPersonaIdentityAdapter:
    ...
    def __init__(self, settings: Settings) -> None:
        ...
        if not settings.profile_explicit:
            raise LocalPersonaProfileError(
                "MKT_CREATIVE_PROFILE is not set, so the local profile was inherited rather "
                "than chosen; the seeded dev personas authenticate nobody and are refused. "
                "Set MKT_CREATIVE_PROFILE=local deliberately for a dev or demo run, or "
                "MKT_CREATIVE_PROFILE=gcp for a real deployment."
            )
