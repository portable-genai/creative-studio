"""Local knowledge-base adapter (KnowledgeBasePort) — SQLite FTS5 brand corpus.

The ``local`` profile's stand-in for **File Search / Agent Search** over the internal brand
/ creative corpus (whose GCP adapter is Agent Search, and whose platform adapter is the
shared A2 Enterprise KB): a ``sqlite3`` database with an **FTS5** virtual table over the
seeded fictional brand passages, queried with BM25 (``ORDER BY rank``). It is SDK-free,
deterministic and seedable, so the same code grounds the offline run and the unit tests.
There is no Google emulator for Agent Search, so this path is unconditional.
...
_DEFAULT_DB_DIR = Path.home() / ".creative_studio"
...
class LocalKnowledgeBaseAdapter:
    """Index + retrieve brand-corpus passages from a local SQLite FTS5 store (BM25)."""
    ...
    def seed(self, passages: tuple[RetrievedPassage, ...] | list[RetrievedPassage]) -> int:
        """Replace the index contents with ``passages`` (deterministic test/run seed)."""
        with self._lock:
    ...
    def search(self, query: RetrievalQuery) -> list[RetrievedPassage]:
        """Return ranked, market/vertical-scoped passages with citations."""
