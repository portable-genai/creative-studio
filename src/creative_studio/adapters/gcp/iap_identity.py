...
class IapIdentityAdapter:
    ...
    def __init__(self, settings: Settings) -> None:
        ...
        # "/projects/<NUM>/apps/<ID>". Configure via MKT_CREATIVE_IAP_AUDIENCE; required in
        # secure mode.
        self._audience = read_env_setting("MKT_CREATIVE_IAP_AUDIENCE").value

    def resolve(self, ctx: RequestContext) -> Principal:
        # The configuration check comes FIRST, before the assertion header is
        # even read, and it always did here. What changed is the TYPE: a plain
        # IdentityError is answered 401 'authentication required', which sends an
        # operator hunting for a credential that would not have helped.
        if not self._audience:
            raise IapAudienceUnconfiguredError(
                "MKT_CREATIVE_IAP_AUDIENCE is not configured, so no IAP assertion "
