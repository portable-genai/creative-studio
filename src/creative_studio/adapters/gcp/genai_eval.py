"""Gen AI evaluation gate adapter (EvaluationGatePort) — the A4 promotion gate for D3.
...
scores the system on the metrics that matter for a brand-safe creative studio (check
groundedness, citation accuracy, brand-safety detection, review safety) and maps the result
onto an :class:`EvalReport` whose ``passed`` flag gates promotion in CI/CD.
