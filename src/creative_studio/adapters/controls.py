...
(``MKT_CREATIVE_GUARDRAIL``, ``MKT_CREATIVE_REVIEW_ROUTING``), the container binds one of these
...
tool, the MCP ``generate_creative`` tool and the CLI) wraps the bound router in
:class:`RecordingReviewRouter` for that one call, so what it returns can say what happened:
``routed``, ``failed``, ``off`` or ``not_required``. A failure is logged and
absorbed here, because an already-audited result must not fail on a console outage, but it is
no longer invisible: the caller reports ``failed``.
...
from ..domain.models import CreativeStudioResult, Direction, GuardrailVerdict
