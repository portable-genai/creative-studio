"""ReviewRouterPort: the boundary that routes an escalated creative result to human-review-console
(rule R8).
...
class ReviewRouterPort(Protocol):
    def route(self, result: CreativeStudioResult, *, maker: str, tenant: str = "") -> None:
        """Route an escalated creative result to human-review-console for human review (idempotent
        per id ideal).
        """
        ...
