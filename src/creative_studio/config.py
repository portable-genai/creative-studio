...
D3 is generic and APAC: the active ``vertical`` (banking | online retail) and ``market``
...
``MKT_CREATIVE_PROFILE`` is NO CHOICE rather than a silent ``local``. That distinction is the
...
PROFILE_ENV = "MKT_CREATIVE_PROFILE"
...
#: What :attr:`Settings.generator_model` reports for a laptop profile whose generator port is
#: bound to the deterministic stub, and what that stub notes as the model that answered, so the
#: console's model pill reads the same string before and after an answer.
...
def resolve_profile(
    ...
    ``MKT_CREATIVE_PROFILE`` wins; naming a profile in ``config/settings.yaml`` is an equally
...
class ModelSettings:
    ...
    triage: str = "gemini-3.5-flash"  # routing / triage / guardrail pre-checks
...
GUARDRAIL_ENV = "MKT_CREATIVE_GUARDRAIL"
REVIEW_ROUTING_ENV = "MKT_CREATIVE_REVIEW_ROUTING"
...
#: rather than a hand-off that fails, silently, on every creative result.
...
class LoggingSettings:
    log_name: str = "creative-studio-audit"
    bucket: str = "creative-studio-worm"
...
class AgentEngineSettings:
    ...
    display_name: str = "creative-studio"
...
class LocalSettings:
    ...
    Empty strings select the per-package default under ``~/.creative_studio/``; tests pass
    ``:memory:`` for ephemeral, deterministic stores. No Google Cloud here.
    """

    db_path: str = ""  # SQLite FTS5 brand-corpus index
    audit_path: str = ""  # append-only audit store
    seed_path: str = ""  # corpus seed JSON ("" => bundled fictional seed)


@dataclass(frozen=True)
class PolicySettings:
    """Adopter-owned creative decision policy with reference defaults."""

    max_exclamations: int = 1
    caps_word_threshold: int = 2
    max_avg_word_length: float = 7.5
    variant_similarity_threshold: float = 0.7

    def __post_init__(self) -> None:
        if self.max_exclamations < 0 or self.caps_word_threshold < 1:
            raise ValueError("brand style thresholds are outside their supported range")
        if self.max_avg_word_length <= 0:
            raise ValueError("max_avg_word_length must be positive")
        if not 0 <= self.variant_similarity_threshold <= 1:
            raise ValueError("variant_similarity_threshold must be within 0..1")
...
class Settings:
    ...
    # gcp | local | platform | onprem. There is NO default: see ``resolve_profile``, where an
    # unset MKT_CREATIVE_PROFILE is no choice rather than a silent ``local``.
    profile: str = "local"
    vertical: str = "banking"  # banking | online_retail (the active vertical)
    market: str = "SG"  # JP | AU | SG (the active market)
    channel: str = "email"  # default channel (CLI/API may override)
    ...
    # sets this False when neither MKT_CREATIVE_PROFILE nor the settings file named one.
    ...
    def load(path: str | os.PathLike[str] | None = None) -> Settings:
        ...
        channel = setting_or_default("MKT_CHANNEL", str(raw.pop("channel", "email")))
...
class Container:
    ...
    @cached_property
    def copy(self) -> Any:
        return self._bind("copy")

    @cached_property
    def image(self) -> Any:
        return self._bind("image")
