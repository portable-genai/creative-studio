...
D3 generates marketing creative from a brief and brand corpus and handles no customer PII
(C2/C3/C4 n/a in the practices audit), so there is no PII-redaction callback here. The
guardrail's own ``sanitized_text`` is still honoured, so any content the policy chooses to mask
never reaches the model or the audit record.

The callbacks are built from a :class:`~creative_studio.config.Container`, so the active profile
decides whether these are real Model Armor / Cloud Logging calls or on-prem placeholders.
