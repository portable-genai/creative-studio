# creative-studio — Creative Studio API service image.
#
...
# is unset, so this image sets MKT_CREATIVE_PROFILE=gcp EXPLICITLY to opt in to the managed
...
HEALTHCHECK --interval=30s --timeout=5s --start-period=20s --retries=3 \
    CMD python -c "import urllib.request,os; urllib.request.urlopen('http://127.0.0.1:'+os.environ.get('PORT','8102')+'/healthz')" || exit 1
