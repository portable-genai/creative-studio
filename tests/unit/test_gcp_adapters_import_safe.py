...
from creative_studio import ports
from creative_studio.config import Settings
from creative_studio.domain.errors import UnsupportedMarketError
from creative_studio.domain.models import Market
...
GCP_ADAPTERS: dict[str, tuple[str, type]] = {
    "copy": (
        "creative_studio.adapters.gcp.gemini_copy:GeminiCopyAdapter",
        ports.CopyGenerationPort,
    ),
    "image": (
        "creative_studio.adapters.gcp.imagen_image:ImagenImageAdapter",
        ports.ImageGenerationPort,
    ),
    "knowledge_base": (
        "creative_studio.adapters.gcp.file_search_kb:FileSearchKnowledgeBaseAdapter",
        ports.KnowledgeBasePort,
    ),
    "guardrail": (
        "creative_studio.adapters.gcp.model_armor_guardrail:ModelArmorGuardrailAdapter",
        ports.GuardrailPort,
    ),
    "audit": (
        "creative_studio.adapters.gcp.cloud_logging_audit:CloudLoggingAuditAdapter",
        ports.AuditSinkPort,
    ),
    "tracer": (
    ...
    "evaluation": (
        "creative_studio.adapters.gcp.genai_eval:GenAiEvalAdapter",
        ports.EvaluationGatePort,
    ),
    "agent_registry": (
        "creative_studio.adapters.gcp.a2a_registry:A2ARegistryAdapter",
        ports.AgentRegistryPort,
    ),
    "tool_catalog": (
        "creative_studio.adapters.gcp.mcp_tool_catalog:McpToolCatalogAdapter",
...
def test_gcp_adapter_constructs_and_satisfies_protocol(port_name: str) -> None:
    dotted, protocol = GCP_ADAPTERS[port_name]
    from creative_studio.config import instantiate

    adapter = instantiate(dotted, _settings())
    assert isinstance(adapter, protocol), f"{dotted} does not satisfy {protocol.__name__}"


def test_importing_gcp_family_does_not_pull_in_google() -> None:
...
def test_config_bindings_cover_gcp_for_every_port() -> None:
    settings = _settings()
...
def test_region_validation_accepts_each_apac_market() -> None:
    from creative_studio.adapters.gcp._region import resolve_region
...
def test_region_validation_rejects_out_of_residency_region() -> None:
    from creative_studio.adapters.gcp._region import resolve_region
