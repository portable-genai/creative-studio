...
The defect this guards was a supply-chain step that excepted a named advisory "until
2026-08-06" via ``[[ "$(date -u +%F)" > "2026-08-06" ]]``. Every failure mode of that
shape is silent. Before the date it downgrades a hard gate to an inline allowlist nobody
re-reads, so an advisory the author never anticipated (here nanoid GHSA-2v37-7h3g-55p8)
fails the build for a reason the comment does not explain. After the date the step starts
failing on a schedule, with no commit and no review to point at. And the pin the exception
leaned on had itself become the vulnerability: ``overrides.postcss = 8.5.22`` held the
package AT the version GHSA-fxqj-rqcc-2cmp flags and stopped the resolver moving past it.
...
about any one advisory. Exceptions belong in a commit a human approves, not in a date
literal that expires unattended.

Scope: the SHELL of each ``run:`` step in every file under ``.github/workflows/``.
Comments and prose may cite dates freely (an advisory's publication date is useful
context); it is the executable text that must not carry one.
"""
...
def _workflow_files() -> list[Path]:
    return sorted(p for p in WORKFLOWS.rglob("*") if p.suffix in {".yaml", ".yml"})


def _run_scripts(workflow: Path) -> list[tuple[str, str]]:
    """Every ``run:`` script in the file, paired with the step name that owns it.

    Parsed rather than grepped: a literal inside a block scalar is indented and wrapped
    however the author left it, and a line-oriented scan cannot tell an executable line
    from the surrounding YAML.
    """
...
def test_the_workflow_directory_is_where_this_test_thinks_it_is() -> None:
    """The control: without it, a moved directory would make the check vacuously green."""
