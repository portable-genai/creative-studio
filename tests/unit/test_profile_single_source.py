...
The defect this locks out is absence read as consent: ``MKT_CREATIVE_PROFILE`` unset used to
...
from creative_studio.adapters.local.identity import (
...
_SRC = Path(__file__).resolve().parents[2] / "src" / "creative_studio"
...
def test_an_EMPTIED_variable_refuses_rather_than_inheriting_the_unset_default() -> None:
    """This case is kept apart from the absent one; folding them together pins the defect.

    Reading unset and emptied as one state is the resolver defect, so a test that asserts they
    are the same can never catch it. An operator who deliberately empties the variable expresses
    an intent that names no profile, so it refuses instead of inheriting.
    """
...
def test_an_unconsented_run_gets_no_cross_origin_trust() -> None:
    ...
    choice = resolve_profile({})
    assert cors_allowlist(choice.exposure_profile, origins_env="MKT_CREATIVE_CORS_ORIGINS") == []
    assert cors_allowlist("local", origins_env="MKT_CREATIVE_CORS_ORIGINS") != []
