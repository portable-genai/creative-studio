...
needs; and every caller that hands a creative result to the review router says what happened to it.
...
from creative_studio.adapters.controls import (
    DisabledGuardrail,
...
from creative_studio.domain.models import Direction
from creative_studio.envread import ConfiguredEmptyError
from tests.conftest import LOOPBACK_PEER

_PROFILE_ENV = "MKT_CREATIVE_PROFILE"
...
def test_a_process_with_a_control_off_says_so_at_startup(
    ...
    with caplog.at_level(logging.WARNING, logger="creative_studio.config"):
...
def test_a_failed_hand_off_is_reported_and_logged_never_raised(
    ...
    failed = RecordingReviewRouter(_Refusing())
    with caplog.at_level(logging.WARNING, logger="creative_studio.adapters.controls"):
        failed.route(object(), maker="m")  # type: ignore[arg-type]
    assert failed.outcome is ReviewRouting.FAILED
...
def _post_creative(monkeypatch: pytest.MonkeyPatch, container: Container) -> dict[str, Any]:
    monkeypatch.setattr(deps, "get_container", lambda: container)
    response = TestClient(app, client=LOOPBACK_PEER).post("/v1/creative", json=_CREATIVE_BODY)
    assert response.status_code == 200, response.text
    body: dict[str, Any] = response.json()
    return body


def test_the_api_reports_a_routed_result(
    monkeypatch: pytest.MonkeyPatch, local_settings: Settings
) -> None:
    body = _post_creative(monkeypatch, _container(local_settings))
...
def test_the_api_reports_routing_off(
    ...
    body = _post_creative(monkeypatch, _container(local_settings, review_routing=False))
    assert body["review_routing"] == "off"


def test_the_api_reports_a_failed_hand_off_instead_of_failing_the_result(
    monkeypatch: pytest.MonkeyPatch, local_settings: Settings
) -> None:
    body = _post_creative(monkeypatch, _container(local_settings, router=_Refusing()))
...
def test_the_agent_tool_reports_the_hand_off(local_settings: Settings) -> None:
    ...
    payload = tools.generate_creative("high-yield savings", settings=settings)
    assert payload["review_routing"] == "off"


def test_the_mcp_generate_tool_reports_the_hand_off(
    monkeypatch: pytest.MonkeyPatch, local_settings: Settings
) -> None:
    from creative_studio.mcp import server as mcp_server

    monkeypatch.setattr(
        deps, "get_container", lambda: _container(local_settings, router=_Refusing())
    )
    payload = mcp_server.build_handlers(actor="svc:test")["generate_creative"](
        topic="high-yield savings", market="SG", vertical="banking", channel="email"
    )
    assert payload["review_routing"] == "failed"


def test_the_cli_says_where_the_result_went(
    monkeypatch: pytest.MonkeyPatch, local_settings: Settings
) -> None:
    from creative_studio.cli.main import app as cli

    monkeypatch.setattr(
        deps, "get_container", lambda: _container(local_settings, review_routing=False)
    )
    result = CliRunner().invoke(cli, ["generate", "high-yield savings"])
