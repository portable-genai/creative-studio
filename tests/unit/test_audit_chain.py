...
from creative_studio.adapters.local.audit import LocalAppendOnlyAuditAdapter
from creative_studio.config import LocalSettings, Settings
from creative_studio.domain.models import AuditEvent, Decision
...
def test_events_round_trip_and_chain_verifies() -> None:
    adapter = _adapter()
    adapter.record(_event("generate_creative"))
    adapter.record(_event("review_variant"))
    events = adapter.read_all()
    assert [e["action"] for e in events] == ["generate_creative", "review_variant"]
...
def test_tampering_is_detected() -> None:
    ...
    conn.execute(
        "UPDATE audit_log SET event_json = replace(event_json, 'generate_creative', 'x') "
        "WHERE seq = 1"
    )
    conn.commit()
