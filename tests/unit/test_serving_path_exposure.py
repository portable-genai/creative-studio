...
Red before the fix, proved by execution: the studio had no app-bound exposure guard at all, and
the shipped entry points both hand the app object straight to uvicorn:

    Dockerfile:  CMD exec uvicorn creative_studio.api.app:app --host 0.0.0.0 --port ${PORT}
    Makefile:    run-api -> $(BIN)/uvicorn creative_studio.api.app:app --host $(API_HOST) ...
...
from creative_studio.adapters.gcp.iap_identity import IapIdentityAdapter
from creative_studio.adapters.local.identity import LocalPersonaIdentityAdapter
from creative_studio.adapters.onprem.identity import OnPremIdentityAdapter
from creative_studio.api import app as app_module
from creative_studio.api import deps
from creative_studio.config import Settings, end_user_auth_kind, identity_adapter_class
from creative_studio.ports.identity import (
    CLIENT_ASSERTED,
    END_USER_AUTH_ATTR,
    END_USER_AUTH_KINDS,
    UNIMPLEMENTED,
...
_APP_SOURCE = _REPO_ROOT / "src" / "creative_studio" / "api" / "app.py"
...
def test_a_lan_peer_is_refused_the_consequential_route_too() -> None:
    ...
    response = _client(LAN_PEER).post(
        "/v1/creative", json={}, headers={"X-Dev-Persona": "approver"}
    )
    assert response.status_code == 503
...
def test_a_loopback_peer_still_gets_the_local_demo(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    """
    monkeypatch.setenv("MKT_CREATIVE_PROFILE", "local")
...
def test_an_unset_profile_advertises_no_personas_rather_than_unresolvable_ones(
    ...
    refuse to resolve.
    """
    monkeypatch.delenv("MKT_CREATIVE_PROFILE", raising=False)
    deps.get_container.cache_clear()
...
def test_the_shipped_entry_points_serve_the_app_object() -> None:
    ...
    assert "creative_studio.api.app:app" in dockerfile
    assert "--host 0.0.0.0" in dockerfile
    assert "creative_studio.api.app:app" in makefile
...
def test_the_posture_follows_a_REBOUND_identity_port_not_the_profile_name() -> None:
    ...
    rebound["identity"]["onprem"] = "creative_studio.adapters.gcp.iap_identity:IapIdentityAdapter"
...
def test_an_unresolvable_binding_fails_CLOSED_rather_than_raising_past_the_guard() -> None:
    ...
    broken["identity"]["local"] = "creative_studio.nope:Missing"
...
_MUTANT = (
    "_TOKEN_ENV = 'MKT_CREATIVE_S2S_TOKEN'\n"
    "_ZERO_SECRET_LOCAL = (\n"
