...
const HERE = "http://console.test/studio";
const API = "http://api.test:8102";
...
test("only URLs under the console's API base count", () => {
  assert.equal(isConsoleApi(API + "/v1/creative", API, HERE), true);
  assert.equal(isConsoleApi(new URL(API + "/healthz"), API, HERE), true);
  assert.equal(isConsoleApi({ url: API + "/v1/x" }, API, HERE), true);
  assert.equal(isConsoleApi("http://elsewhere.test/v1/creative", API, HERE), false);
  assert.equal(isConsoleApi("/v1/creative", API, HERE), false, "the page's own origin is not the API");
  ...
  assert.equal(isConsoleApi("/svc/api/v1/creative", "/svc/api", HERE), true);
  assert.equal(isConsoleApi("/svc/apix/v1/creative", "/svc/api", HERE), false);
});
...
test("the wrapper reports an answer from the console's API and ignores other origins", async () => {
  ...
  await window.fetch(API + "/v1/creative", { method: "POST" });
  await window.fetch("http://elsewhere.test/v1/creative");
  stop();
...
test("the wrapper is installed once however many listeners, and restored by the last", async () => {
  ...
  await window.fetch(API + "/v1/creative");
  stopFirst();
