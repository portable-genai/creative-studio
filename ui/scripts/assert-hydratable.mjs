...
// the CSP string, and the string was right. `tsc --noEmit` was clean. `next build` succeeded. The
// page rendered, the headers were correct, and a screenshot looked exactly like a working
// console. What was shipped was dead markup: `script-src 'self'` (or, here, no `script-src` at
// all under a `default-src`) blocks Next's inline hydration bootstrap, `__next_f` never fills,
// React never attaches, and no control on the page does anything.
//
...
//
//   1. The policy is complete: every directive this console depends on is present, and none of
//      them is empty (a valueless directive is a parse error browsers discard, which is how a
//      frame-ancestors-only console loses its clickjacking control without a trace).
...
// fallback, that combination blocks strictly MORE than the unfixed policy did. The header is
// byte-identical in the working and the broken case. Only the markup knows.
//
...
/** Directives the console's policy must actually carry. Absence is the fleet defect. */
